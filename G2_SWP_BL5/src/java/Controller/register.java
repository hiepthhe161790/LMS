...
public class register extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // Additional logic to handle file upload (avatar) goes here
        // Create User object and perform user registration logic
        User user = new User();
        ...
        if (password.equals(confirmPassword)) {
            if (!userDAO.checkEmail(email)) {
                if (userDAO.createUser(user)) {
                    ...
                    String emailToken = md5.encryptAES(email);
                    String to = email;
                    ...
                    try {
                        ...
                        message.setText("Hi, for security, please verify your account with the link below. "
                                + "Your OTP is: http://localhost:9999/G2_SWP_BL5/verifyUser?emailToken=" + emailToken
                        );
                        Transport.send(message);
                        request.setAttribute("errmsg", "Please check your email to confirm");
                        request.getRequestDispatcher("register.jsp").forward(request, response);
