            ...
            if (existingUser != null) {
                ...
                try {
                    ...
                    message.setText("Hi, for security, please verify your account with the OPT below. "
                            ...
                            + "http://localhost:9999/G2_SWP_BL5/ValidateOtp?OTP=" + randomOtp);
                    Transport.send(message);
                    Logger.getLogger(ForgotPassword.class.getName()).log(Level.INFO, "Email sent successfully");
                } catch (MessagingException e) {
                    Logger.getLogger(ForgotPassword.class.getName()).log(Level.SEVERE, "Failed to send email: " + e.getMessage());
                ...
                request.setAttribute("message", "Email does not exist in the database. Please provide a registered email.");
                dispatcher = request.getRequestDispatcher("forgotPassword.jsp");
                dispatcher.forward(request, response);
            }
        } else {
            // Handle invalid email input
            request.setAttribute("message", "Invalid email format. Please provide a valid email.");
            dispatcher = request.getRequestDispatcher("forgotPassword.jsp");
            dispatcher.forward(request, response);
        }
    }
